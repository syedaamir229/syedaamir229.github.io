...
## Scope: this repo owns the site

Everything that ends up at a URL on syedaamir.com lives here, and nothing else does. In August 2026 the off-site content tooling moved to a separate private repo (the "content hub"): the LinkedIn drafts and companion spec, the editorial calendar, the LinkedIn image renderers, and the three `linkedin-*` skills. This repo is public, so the hub is referenced by role, not by path.

Two consequences worth knowing before you work here:

- **Writing a blog post no longer ends with writing its LinkedIn companion.** `blog-writeup` stops when the post lands. Distribution is a separate session in the hub. That checkpoint is deliberate: publishing to the site and promoting it are different decisions.
- **The hub reads from this repo, never the reverse.** The brand palette (the `--color-*` custom properties in [src/styles/global.css](src/styles/global.css)) and the Inter files in [public/fonts/](public/fonts/) are read at render time by the hub's image renderer, so the images cannot drift from the site. Renaming a color token or moving those fonts breaks image builds there. That failure surfaces in the hub, not in `npm run build` here, so treat both as a published interface rather than internal details.
...
- **Per-surface voice specs**: [docs/SITE.md](docs/SITE.md) (landing), [docs/BLOG.md](docs/BLOG.md) (per blog post), [docs/PROJECTS.md](docs/PROJECTS.md) (per project case study). The LinkedIn companion spec moved to the content hub and still inherits tone from `BRAND-voice.md` here.
