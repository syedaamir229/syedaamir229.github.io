...
- **Vendor referents in body vs Tech Stack**: when a category has essentially two peers (e.g. CleverTap or Braze), an inline pair is fine. When a category has several legitimate peers (customer-engagement platforms, BI tools, content-metadata systems), use the generic referent in body ("the customer-engagement platform") and reserve the peer list for Tech Stack ("CleverTap, or peers like Braze and MoEngage"). Body prose should not repeat parenthetical vendor pairings across multiple paragraphs; that turns the prose into commercial copy. Tech Stack is the place where hiring managers anchor on specific tools.
