...
description: >-
  ...
  own). It ends when the post lands: distribution is a separate session
  elsewhere.
---
...
- Structure and confidentiality: `docs/BLOG.md` (spine in section 4, H3 format in
  section 5, confidentiality in section 10).
- The work queue is **not in this repo.** The editorial calendar and the per-post
  briefs (slug, reach, scrub notes, source case study) live in the content hub.
  Aamir brings the brief into the session; do not go looking for a backlog file
  here, and do not invent a slug or a schedule slot on your own.
...
- Read: BRAND-voice.md long-form voice, BLOG.md, the brief Aamir brings from the
  hub, the source case study it names, and the existing old post if this is a
  rewrite. If no brief was given, ask for one rather than guessing the angle:
  the hub is where the decision to write this post was made.
...
- Remind Aamir to mark the post Done on the calendar in the hub. That file is
  not reachable from this repo, so it cannot be updated from this session.
...
- **Stop here.** The LinkedIn companion and the feed portrait are built in the
  content hub, by skills that do not resolve from this repo. Tell Aamir the post
  is staged and that distribution is the next session, over there. Do not draft
  companion copy in this session as a shortcut: the companion is written against
  the finished portrait, and the spec that governs it is not loaded here.
