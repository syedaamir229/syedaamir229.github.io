...
3. Fill frontmatter per [docs/BLOG.md section 3](../docs/BLOG.md#3-frontmatter). `categories` must be in [data/categories.ts](data/categories.ts) `BLOG_CATEGORIES`. `og_title` is required and follows the hook rubric in [docs/BLOG.md section 3](../docs/BLOG.md#3-frontmatter); the schema enforces 8 to 42 characters. Set `date` to the next free Thursday slot in the schedule (see [docs/BLOG.md section 11](../docs/BLOG.md#11-publishing-cadence-and-distribution)); a future date keeps the post hidden until that day.
...
6. Stop. The LinkedIn companion is no longer written here: it lives in the content hub, along with its spec and the feed-image renderer. Publishing the post and promoting it are separate decisions, so they are separate sessions.
