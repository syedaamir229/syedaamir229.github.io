...
GAM API rate limits constrained ingestion scheduling, ad impressions settled over a 14-day attribution window requiring weekly historical refreshes on top of daily runs, and three teams (Ad Ops, Growth, Data) needed different views of the same underlying data from a shared pipeline.
...
- Build separate pipelines per team (independent, but duplicates GAM pulls and derivation logic)
...
![Ad inventory and revenue pipeline architecture: Google Ad Manager API to S3 raw storage to Databricks staging, branching into Inventory, Impressions, VAST Errors, and Delivery Pacing pipelines that output to Power BI dashboards and Slack alerts.](/assets/projects/ad-pipeline.svg)
